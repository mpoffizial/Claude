...
def pstr(p: Params) -> str:
    vol = f"vol=({p.vol_low},{p.vol_high})" if p.vol_filter else "vol=off"
    return (f"entry={p.entry_len} exit={p.exit_len} trail={p.trail_mult} "
            f"sl={p.sl_mult} ema={p.ema_len} cd={p.cooldown} {vol}"
...
def main():
    ...
    fine = {
        "entry_len": sorted({max(12, best.entry_len - 24), best.entry_len,
                             best.entry_len + 24}),
        "exit_len": sorted({max(12, best.exit_len - 24), best.exit_len,
                            best.exit_len + 24}),
    ...
    for seed in WF_SEEDS:
        ...
        for fs in fold_starts:
            ...
            # every candidate is also scored on every test fold, so the
            # final pick rewards consistency, not fold-lottery wins
            for p in candidates:
                cand_fold_scores[pstr(p)].append(
                    robust_score(backtest(df.iloc[te_sl], p)))
